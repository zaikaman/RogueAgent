...
CREATE TABLE public.futures_agents (
  ...
  custom_prompt text,
  stats jsonb NOT NULL DEFAULT '{"mainnet": {"total_trades": 0, "trades_today": 0, "last_trade_at": null, "losing_trades": 0, "total_pnl_usd": 0, "winning_trades": 0, "total_pnl_percent": 0, "max_drawdown_percent": 0}, "testnet": {"total_trades": 0, "trades_today": 0, "last_trade_at": null, "losing_trades": 0, "total_pnl_usd": 0, "winning_trades": 0, "total_pnl_percent": 0, "max_drawdown_percent": 0}}'::jsonb,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
...
CREATE TABLE public.futures_trades (
  ...
  pnl_usd numeric,
  pnl_percent numeric,
  status text NOT NULL DEFAULT 'open'::text CHECK (status = ANY (ARRAY['pending'::text, 'open'::text, 'tp_hit'::text, 'sl_hit'::text, 'closed'::text, 'error'::text])),
  entry_order_id text NOT NULL,
  tp_order_id text,
  sl_order_id text,
  pending_tp_price numeric,
  pending_sl_price numeric,
  error_message text,
  network_mode text NOT NULL DEFAULT 'testnet'::text CHECK (network_mode = ANY (ARRAY['mainnet'::text, 'testnet'::text])),
  opened_at timestamp with time zone NOT NULL DEFAULT now(),
